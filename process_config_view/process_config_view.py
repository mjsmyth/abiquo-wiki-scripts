...
#	ui_path = input("Language file path: ").strip()
	...
	apiAuth = input("Enter API authorization: ").strip()
	apiIP = input("Enter API address: ").strip()
...
# TODO: store if value is a checkbox
	...
	out_file.write ("h3. Configuration")
...
#			subheader = "h4. " + tgso + " properties")	
			subheader = "|| h4. " + tgso + "Configuration element || Default || Notes || "
	...
	for sgp,swgg in enumerate(store_wiki_gui_order):
		...
		if lgs != check_last_group:
			...
			if lgso == "Apps_library":
				lgso = "Apps library"		
			subheader_wiki_links =  "|| h4. " + lgso + "Wiki link || Default || "
