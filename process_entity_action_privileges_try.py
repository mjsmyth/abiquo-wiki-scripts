...
def main():
    camelToUpperSnakePattern = re.compile(r'(?<!^)(?=[A-Z])')
...
#    RS="public static final class|public static final Action|Master"
    RS = "public interface ActionType|public static class |public final static Class\<.*?\>"
    KeyTotal = " "
    NRSave = 0
    EntityName = ""
    ActionName = ""
    ...
    entity_action_list_git_dir = "../platform/api/src/main/java/com/abiquo/api/tracer/action"
    entity_action_git_file = "ActionType.java"
...
#           substitute the dot for an underscore
    ...
    for NR,ear in enumerate(ea_records):
        ...
        if re.match ("\*",ear):
            # it's a comment line so discard it
            #           print ("ear: ",ear)
            break

        if "=" in ear:
            newActionEntityAction = ear.split("=")
            newActionName = newActionEntityAction[0].strip(" ")
            newEntityActionCamelCase = newActionEntityAction[1].strip(" ")
            newEntityAction = camelToUpperSnakePattern.sub('_', newEntityActionCamelCase).upper()
            newEntityName = re.sub(newActionName, "", newEntityAction)
            ...
            # Create a dictionary of entities for output as list
            entity_entity[newEntityName] = newEntityName[:]
            ActionName = newActionName[:]
            entity_space_action[NR] = newEntityName + " " + newActionName
            entity_underscore_actions[NR] = newEntityName + "_" + newActionName
            entity_underscore_action = newEntityName + "_" + newActionName
            entity_dot_action = newEntityName + "." + newActionName
