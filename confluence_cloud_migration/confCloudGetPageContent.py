...
def save_pages(results, confluence_cloud, cloud_site_URL, output_path):
    for page in results:
        ...
        # Write URL of the page as first line, with asterisks for easy grepping
        apiUrl = 'https://' + cloud_site_URL
        page_file.write("**" + apiUrl + page["_links"]["webui"] + "**\n")

        # Write page content to file, removing weird characters e.g. a BOM
        safe_content = str(page_content_unsafe.encode('ascii',
                                                      'xmlcharrefreplace'))
...
def main():
    ...
    start_next = 0
    # cloud_results = get_a_few_pages(confluence_cloud, start_next, spacekey)
    ...
    save_pages(cloud_results, confluence_cloud,
               cloud_site_URL, output_path)
