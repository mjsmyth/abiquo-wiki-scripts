...
#
# Version for User's Guide with Message ID and Message Text and Message Label
#
...
# 
# ./process_api_error_user_guide_table_format.sh input_files/APIError.java.txt output_files/wiki_api_error_user_guide_table_format.txt
# 
# Also creates a separate file with a JSON version of the UI labels for translation
# 
...
{ 
   ...
   if (NF > 1)
   {
      ...
      print "\"" message_id[NR] "\":\"" message_txt[NR] "\"," >> "output_files/APIError_lang_enUS.json.txt"
   }
...
END {
   print "|| Internal Message ID {color:#efefef}__________________{color}|| Message {color:#efefef}____________________________________________________________{color} ||  Info ||"; 
   for (s=1; s<=NR; s++) 
      {if (output_line[s] !~ "NOPRINT") 
         {print output_line[s];}}}' >$2 
